...
void process_directory(const char *inputDir, const char *outputDir, int statsFile, char c) {
            ...
            if (S_ISREG(fileStat.st_mode)) {
                ...
                if (ext && strcmp(ext, ".bmp") == 0) {
                    ...
                    if (pid_calc == 0) {
                        ...
                        char cString[2] = {c, '\0'};
                        execl("/bin/bash", "bash", "./script.sh", cString, (char *)NULL);
                        exit(1);
                    } else {
                        close(pipefd2[0]);
...
int main(int argc, char *argv[]) {
    ...
    process_directory(argv[1], argv[2], statsFile, argv[3][0]);
